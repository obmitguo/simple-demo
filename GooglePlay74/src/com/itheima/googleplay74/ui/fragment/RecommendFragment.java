...
public class RecommendFragment extends BaseFragment {
	...
	public View onCreateSuccessView(){
		...
		//处设置甩动换页
		ShakeListener shark = new ShakeListener(UIUtils.getContext());
		shark.setOnShakeListener(new OnShakeListener() {
			public void onShake() {
				stellar.zoomIn(); //显示下一页
			}
		});
		return stellar;
	}
	...
	public class RecommendAdapter implements StellarMap.Adapter {
		...
		public int getCount(int group) {
			...
			if(group==getGroupCount()-1){
				//最后一页，将除不尽，余下的数量追加在最后一页，保证数据完整不丢失
		...
		public View getView(int group, int position, View convertView) {
			...
			view.setOnClickListener(new OnClickListener() {
				public void onClick(View v) {
					Toast.makeText(UIUtils.getContext(), keyword, 0).show();
				}
			});
			return view;
		}
